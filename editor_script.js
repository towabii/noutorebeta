document.addEventListener('DOMContentLoaded', () => {
    ...
    const levelTabs = document.querySelectorAll('.level-tab')
      , levelIndicator = document.getElementById('current-level-indicator');
    const widthInput = document.getElementById('grid-width')
      , resizeBtn = document.getElementById('btn-resize');
    const deleteSelectionBtn = document.getElementById('btn-delete-selection')
      , selectionInfo = document.getElementById('selection-info');
    const pasteArea = document.getElementById('paste-area')
      , loadFromPasteBtn = document.getElementById('btn-load-from-paste');
    ...
    const OBJECT_COLORS = {
        '#': 'rgba(0, 255, 255, 0.7)',
        '^': 'rgba(255, 0, 0, 0.8)',
        'v': 'rgba(255, 100, 100, 0.8)',
        'J': 'rgba(255, 255, 0, 0.9)',
        'O': 'rgba(255, 255, 0, 0.5)',
        'S': 'rgba(0, 255, 0, 0.5)',
        'C': 'rgba(255, 128, 0, 0.5)',
        'V': 'rgba(255, 0, 255, 0.5)',
        'N': 'rgba(255, 255, 0, 0.5)',
        'G': 'rgba(255, 255, 0, 0.4)',
    ...
    function handlePasteLoad() {
        ...
        if (!content.trim()) {
            loadFeedback.textContent = '❌ テキストエリアが空です。';
    ...
    function parseLevelsJs(content) {
            ...
            if (blockMatch && typeof blockMatch[1] === 'string') {
                ...
                if (lines.length > 0) {
                    ...
                    if (isValid) {
                        ...
                        throw new Error(`レベル ${i} のデータが不正です（行の長さが異なります）。`);
                    }
        ...
        if (Object.keys(parsedData).length === 0)
            throw new Error("カスタムレベルのデータが見つかりません。");
        return parsedData;
    }
    ...
    function setToolMode(mode) {
        toolMode = mode;
        toolModeBtns.forEach(btn => btn.classList.toggle('selected', btn.dataset.mode === mode));
        clearSelection();
        updateCursor();
    }
    function switchLevel(level) {
        currentEditingLevel = level;
        levelTabs.forEach(tab => tab.classList.toggle('selected', parseInt(tab.dataset.level) === level));
        levelIndicator.textContent = level;
        widthInput.value = allLevelsData[level].width;
        canvas.width = allLevelsData[level].width * TILE_SIZE;
    ...
    function resizeCurrentLevel() {
        ...
        allLevelsData[currentEditingLevel] = {
            width: newWidth,
            data: newData
        };
        switchLevel(currentEditingLevel);
    ...
    function handleMouseDown(e) {
        isMouseDown = true;
        startPos = getMouseGridPos(e);
        if (toolMode === 'select') {
            if (selection.active && isPosInSelection(startPos)) {
                isDraggingSelection = true;
                copySelectionToClipboard();
            } else {
                clearSelection();
                selection.active = true;
                selection.x1 = startPos.x;
                selection.y1 = startPos.y;
            }
        }
    }
    function handleMouseMove(e) {
        if (!isMouseDown)
            return;
        const currentPos = getMouseGridPos(e);
        updateCursor(e);
        switch (toolMode) {
        case 'pencil':
            placeObject(currentPos.x, currentPos.y, selectedChar);
            break;
        case 'rectangle':
        case 'select':
            if (!isDraggingSelection) {
                selection.x2 = currentPos.x;
                selection.y2 = currentPos.y;
            }
            break;
        }
        draw();
    ...
    function handleMouseUp(e) {
        if (!isMouseDown)
            return;
        isMouseDown = false;
        const endPos = getMouseGridPos(e);
        switch (toolMode) {
        case 'rectangle':
            applyRectangle(selectedChar);
            clearSelection(false);
            break;
        case 'select':
            if (isDraggingSelection) {
                pasteClipboard(endPos);
            }
            isDraggingSelection = false;
            updateSelectionUI();
            break;
    ...
    function handleMouseLeave() {
        if (isMouseDown) {
            handleMouseUp({
                offsetX: lastMousePos.x,
                offsetY: lastMousePos.y
            });
        }
    }
    function placeObject(x, y, char) {
        const grid = allLevelsData[currentEditingLevel].data;
        if (y >= 0 && y < GRID_HEIGHT && grid.length > 0 && x >= 0 && x < grid[0].length) {
            grid[y][x] = char;
        }
    }
    function applyRectangle(char) {
        const {x1, y1, x2, y2} = getNormalizedSelection();
        for (let y = y1; y <= y2; y++)
            for (let x = x1; x <= x2; x++)
                placeObject(x, y, char);
    }
    ...
    function copySelectionToClipboard() {
        ...
        const grid = allLevelsData[currentEditingLevel].data;
        for (let y = y1; y <= y2; y++)
            clipboard.data.push(grid[y].slice(x1, x2 + 1));
    }
    function pasteClipboard(endPos) {
        ...
        for (let y = y1; y <= y2; y++)
            for (let x = x1; x <= x2; x++)
                grid[y][x] = ' ';
        const dx = endPos.x - clipboard.start.x
          , dy = endPos.y - clipboard.start.y;
        for (let y = 0; y < clipboard.height; y++)
            for (let x = 0; x < clipboard.width; x++) {
                const char = clipboard.data[y][x];
                if (char !== ' ')
                    placeObject(x1 + x + dx, y1 + y + dy, char);
            }
        selection.x1 += dx;
        selection.y1 += dy;
        selection.x2 += dx;
    ...
    function draw() {
        ...
        drawGrid();
        drawObjects();
        if (isMouseDown && (toolMode === 'rectangle' || (toolMode === 'select' && !isDraggingSelection))) {
            drawSelectionBox(selection, 'rgba(0, 100, 255, 0.3)');
        } else if (selection.active && !isDraggingSelection) {
            drawSelectionBox(selection, 'rgba(0, 100, 255, 0.5)');
        }
        if (isDraggingSelection) {
            const currentPos = getMouseGridPos({
                offsetX: lastMousePos.x,
                offsetY: lastMousePos.y
            });
    ...
    function drawObjects() {
        const level = allLevelsData[currentEditingLevel];
        if (!level || !level.data || level.data.length !== GRID_HEIGHT)
            return;
        const grid = level.data;
        for (let r = 0; r < GRID_HEIGHT; r++) {
            for (let c = 0; c < level.width; c++) {
                const char = grid[r] ? grid[r][c] : ' ';
                if (char && char !== ' ')
                    drawTile(c, r, char, 1.0);
            }
        }
    }
    function drawGhost(dx, dy) {
        if (!clipboard)
            return;
        const {x1, y1} = getNormalizedSelection();
        for (let y = 0; y < clipboard.height; y++)
            for (let x = 0; x < clipboard.width; x++) {
                const char = clipboard.data[y][x];
                if (char !== ' ')
                    drawTile(x1 + x + dx, y1 + y + dy, char, 0.5);
            }
    }
    function drawTile(x, y, char, alpha) {
        ctx.globalAlpha = alpha;
        ctx.fillStyle = OBJECT_COLORS[char] || '#fff';
        ...
        if (char === '^') {
            ...
            ctx.fillRect(px + 4, py + TILE_SIZE * 0.7, TILE_SIZE - 8, TILE_SIZE * 0.3);
        } else if (char === 'O') {
            ctx.beginPath();
            ...
            ctx.fillRect(px + TILE_SIZE * 0.25, py, TILE_SIZE * 0.5, TILE_SIZE);
        } else {
    ...
    function drawSelectionBox(sel, color) {
        const {x1, y1, x2, y2} = getNormalizedSelection();
    ...
    function updateCursor(e) {
        const pos = e ? getMouseGridPos(e) : getMouseGridPos({
            offsetX: lastMousePos.x,
            offsetY: lastMousePos.y
        });
        if (toolMode === 'select' && selection.active && isPosInSelection(pos)) {
    ...
    function updateSelectionUI() {
        deleteSelectionBtn.disabled = !selection.active;
        if (selection.active) {
            const {x1, y1, x2, y2} = getNormalizedSelection();
            selectionInfo.textContent = `範囲: (${x1}, ${y1}) - (${x2}, ${y2})`;
        } else {
            selectionInfo.textContent = '「選択」モードで範囲を指定してください';
        }
    }
    ...
    function generateFullCode() {
        ...
        for (let i = 1; i <= MAX_LEVELS; i++) {
            ...
            customLevelsStrings.push(`const CUSTOM_LEVEL_${i} = [\n${lines}\n];`);
        }

        // ★★★ Updated to generate level selection up to MAX_LEVELS ★★★
        const levelBuilderCases = [];
        for (let i = 1; i <= MAX_LEVELS; i++) {
            levelBuilderCases.push(`    if (levelIndex === ${i}) {\n        parts.push(CUSTOM_LEVEL_${i});\n    }`);
        }
...
`;
    ...
    function copyToClipboard() {
        if (!outputArea.value) {
            copyFeedback.textContent = '先にコードを生成してください。';
            return;
        }
        outputArea.select();
